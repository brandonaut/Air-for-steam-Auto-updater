...
try:
    latest_release_info = requests.get('https://api.github.com/repos/outsetini/Air-for-Steam/releases/latest')
except(Exception) as exception:
    print(exception)
    sys.exit(1)

#For log file write at EOF
updatetag = latest_release_info.json()['tag_name']

if os.path.isfile(logfile):
    try:
        with open(logfile, 'r+') as updatelog:
            if updatelog.read() == updatetag:
                print('No update found, quitting')
                sys.exit(0)
    except IOError:
        print('An error occurred trying to read the logfile. Please check write permission.')
        sys.exit(1)
...
try:
    latest_release = requests.get(latest_release_info.json()['zipball_url'])
except(Exception) as exception:
    print(exception)
    sys.exit(1)
...
try:
    with open(os.path.join(skinsDir, 'temp.zip'), 'wb') as temp:
        temp.write(latest_release.content)
except(Exception) as exception:
...
with open(logfile, 'w') as updatelog:
    try:
        ...
        print('An error occurred writing to logfile, quitting...')
        sys.exit(1)
